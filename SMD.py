...
def SMD(X,k_guess, trials = None,n_sub = None,prop_algo = 'agglo', class_algo = 'entropy', cluster_prior_min=None):
	...
	ray_jobs = [get_classifier_dims(X,n_sub, cluster_prior, cluster_algo) for _ in range(trials)]
...
def find_classifier_dims_entropy(X,cell_sample,k_sub, cluster_algo):
	...
	gnout = [one_tree.remote(Xit,k_guess,ikk1,ikk2) for ikk1 in np.unique(k_guess) for ikk2 in np.unique(k_guess) if ikk1<ikk2 ]
	return gnout
...
def find_classifier_dims_maxmargin(X,cell_sample,k_sub, cluster_algo):
	...
	Xit = X[cell_use,:]
	k_guess = cluster_algo(k_sub_i).fit_predict(Xit)
	gnout = [one_plane.remote(Xit,k_guess,ikk1,ikk2) for ikk1 in np.unique(k_guess) for ikk2 in np.unique(k_guess) if ikk1<ikk2 ]
	return gnout
